...
| `wan_teacher_leftover_smoke` | `wan_notta` / always / gated / nwarp / nwarp_live / pwarp / pwarp_live. Leftover Panda n=2. Official teacher, no `self_forcing_dmd.pt`. | **FAILED 2:0 17132796–802.** `assert FLASH_ATTN_2_AVAILABLE`. n=2 n_ok=0. Protocol else OK (`host=wan_teacher`, panda_0000/0001). | SDPA patch in `run_wan_teacher.py`. Resubmit smoke only. |
| `wan_teacher_moviegen_smoke` | Same 7 methods. MovieGen n=2. Official teacher, 81 frames. | **FAILED 2:0 17132804–810.** Same FA2 assert. moviegen_000/001. | Do not compile flash-attn. Do not launch 128. |
