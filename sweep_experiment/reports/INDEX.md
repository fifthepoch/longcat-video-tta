...
| `wan_teacher_leftover_smoke` | `wan_notta` / always / gated / nwarp / nwarp_live / pwarp / pwarp_live. Leftover Panda n=2. Official teacher, no `self_forcing_dmd.pt`. | **IN-FLIGHT 17132796–803.** Flow = leftover pixels. Sidecar `host=wan_teacher` `source=leftover` `prefix=flow_only`. Cite `wan_notta`. | Harvest after COMPLETED 0:0: `harvest_wan_teacher.py --series wan_teacher_leftover_smoke`. |
| `wan_teacher_moviegen_smoke` | Same 7 methods. MovieGen n=2. Official teacher, 81 frames. | **IN-FLIGHT 17132804–811.** Flow = first `wan_notta` clip. Sidecar `source=t2v_firstseg`. Do not mix leftover dirs. Do not launch 128. | Harvest: `harvest_wan_teacher.py --series wan_teacher_moviegen_smoke`. |
