...
| `wan_teacher_leftover_smoke` | `wan_notta` / always / gated / nwarp / nwarp_live / pwarp / pwarp_live. Leftover Panda n=2. Official teacher, no `self_forcing_dmd.pt`. | **COMPLETED 0:0 17135846–853** (SDPA). Harvest pending (cwd miss from `~`). First wave FA2 **FAILED 2:0**. | Harvest from repo root. Do not letter n=2. |
| `wan_teacher_moviegen_smoke` | Same 7 methods. MovieGen n=2. Official teacher, 81 frames. | **COMPLETED 0:0 17135854–861** (SDPA). Harvest pending. | Same harvest paste. Do not launch 128. |
